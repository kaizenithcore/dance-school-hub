...
export default function SchoolLandingPage() {
  ...
  const hasOptimizedWebsite = Boolean(formData?.publicProfile?.hasOptimizedWebsite ?? formData?.hasOptimizedWebsite);
  ...
  return (
    <div className={`animate-fade-in ${hasOptimizedWebsite ? "public-optimized" : ""}`}>
      {isDemo && <DemoBanner />}
      {isDemo && <DemoTourCard />}
      ...
      <section className="relative overflow-hidden border-b border-border">
        <div className={`absolute inset-0 ${hasOptimizedWebsite ? "bg-gradient-to-br from-primary/10 via-accent/20 to-background" : "bg-gradient-to-br from-primary/5 via-transparent to-accent/30"}`} />
        <div className="container relative py-20 sm:py-28">
          <div className="max-w-2xl">
            ...
            <p className="mt-5 text-lg text-muted-foreground leading-relaxed max-w-lg">
              {formData?.publicProfile?.tagline || formData?.publicProfile?.description || "Descubre nuestras clases, elige tu horario y reserva tu plaza en menos de 2 minutos."}
            </p>
