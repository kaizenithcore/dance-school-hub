...
const proofBadges = [
  ...
  "Resultados medibles",
  "Bloque escalable para nuevos testimonios",
];
...
export function Credibility() {
  const published = landingTestimonials.find((item) => item.status === "published");
  const upcoming = landingTestimonials.filter((item) => item.status === "placeholder");
      ...
      <div className="container">
        ...
        >
          ...
          <p className="mt-4 text-muted-foreground">
            Empezamos con un caso publicado y una estructura lista para ampliar con nuevos testimonios a medida que se cierren.
          </p>
        </motion.div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-accent text-primary font-bold">
                  {published.logoPlaceholderText}
                </div>
                <div>
                  <p className="text-base font-semibold text-foreground">{published.schoolName}</p>
                  <p className="text-sm text-muted-foreground">{published.location}</p>
                </div>
              </div>

              <div className="rounded-xl border border-success/30 bg-success/5 px-4 py-3 lg:min-w-[280px]">
                <p className="text-xs uppercase tracking-wide text-muted-foreground">{published.metricLabel}</p>
                <p className="mt-1 text-xl font-bold text-success">{published.metricValue}</p>
              </div>
            </div>

            <div className="mt-6 rounded-xl border border-border bg-background p-5">
              <Quote className="h-5 w-5 text-primary mb-3" />
              <p className="text-foreground leading-relaxed">{published.quote}</p>

              <div className="mt-5 flex items-center gap-3">
                <Avatar className="h-10 w-10 border border-border">
                  {published.avatarUrl ? <AvatarImage src={published.avatarUrl} alt={published.personName} /> : null}
                  <AvatarFallback>{getInitials(published.personName)}</AvatarFallback>
                </Avatar>
                <div>
                  <p className="text-sm font-semibold text-foreground">{published.personName}</p>
                  <p className="text-xs text-muted-foreground">{published.personRole}</p>
                </div>
              </div>
            </div>
          </motion.article>
          ...
          {upcoming.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">Caso en preparacion</p>
                  <p className="text-xs text-muted-foreground">Espacio para nuevo testimonio</p>
                </div>
              </div>
