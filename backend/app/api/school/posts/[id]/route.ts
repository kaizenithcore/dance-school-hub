...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const data = await portalFoundationService.updateFeedPost(auth.context.tenantId, id, parsed.data);
    return ok(data, 200, origin);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to update school post";
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  try {
    const { id } = await context.params;
    const data = await portalFoundationService.deleteFeedPost(auth.context.tenantId, id);
    return ok(data, 200, origin);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to delete school post";
