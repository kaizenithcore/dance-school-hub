...
import { createAlbumSchema } from "@/lib/validators/portalFoundationSchemas";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await portalFoundationService.listAlbums(auth.context.tenantId);
    return ok(data, 200, origin);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to list albums";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const data = await portalFoundationService.createAlbum(auth.context.tenantId, auth.user.id, parsed.data);
