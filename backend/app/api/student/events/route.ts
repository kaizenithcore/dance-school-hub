...
import { listStudentEventsSchema } from "@/lib/validators/studentPortalSchemas";
...
export async function GET(request: NextRequest) {
  ...
  const parsed = listStudentEventsSchema.safeParse({
  ...
  try {
    const result = await studentPortalService.listStudentEvents(auth.userId, parsed.data);
    return ok(result, 200, origin);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to fetch student events";
