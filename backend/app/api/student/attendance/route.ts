...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = await studentPortalService.getStudentAttendanceStats(auth.userId);
    return ok(result, 200, origin);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to fetch student attendance stats";
