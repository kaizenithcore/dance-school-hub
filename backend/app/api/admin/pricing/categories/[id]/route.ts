...
// PUT /api/admin/pricing/categories/[id] - Update a discipline category
export async function PUT(
  ...
  try {
    ...
    if (!existingCategory || existingCategory.tenant_id !== membership.tenant_id) {
      return withCors(request, { error: 'Categoría no encontrada' }, 404)
    }
    ...
    const category = await pricingService.updateDisciplineCategory(id, { ...body, tenant_id: membership.tenant_id })

    return withCors(request, { category })
  } catch (error: unknown) {
    console.error('Error updating discipline category:', error)
...
// DELETE /api/admin/pricing/categories/[id] - Delete a discipline category
export async function DELETE(
  ...
  try {
    ...
    await pricingService.deleteDisciplineCategory(id)

    return withCors(request, { success: true })
  } catch (error: unknown) {
    console.error('Error deleting discipline category:', error)
    if (error instanceof Error && error.message === 'unauthorized') {
      return withCors(request, { error: 'No autorizado' }, 401)
    }
    if (error instanceof Error && error.message === 'forbidden') {
