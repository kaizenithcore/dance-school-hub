...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const data = await portalFoundationService.updateTeacherPostApproval(auth.context.tenantId, id, "rejected", notes);
    return ok(data, 200, origin);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to reject teacher post";
