...
import { updateRoomSchema } from "@/lib/validators/roomSchemas";
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const room = await roomService.getRoom(auth.context.tenantId, id);
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  try {
    const { id } = await context.params;
    await roomService.deleteRoom(auth.context.tenantId, id);
