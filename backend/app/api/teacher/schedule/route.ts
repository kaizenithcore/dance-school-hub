...
import { teacherScheduleQuerySchema } from "@/lib/validators/portalFoundationSchemas";
...
export async function GET(request: NextRequest) {
  ...
  const parsed = teacherScheduleQuerySchema.safeParse({
  ...
  try {
    const data = await portalFoundationService.listTeacherSchedule(auth.userId, auth.email, parsed.data.tenantId);
