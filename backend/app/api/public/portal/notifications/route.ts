...
import { listNotificationsSchema } from "@/lib/validators/portalFoundationSchemas";
...
export async function GET(request: NextRequest) {
  ...
  const parsed = listNotificationsSchema.safeParse({
    ...
    onlyUnread: request.nextUrl.searchParams.get("onlyUnread") ?? undefined,
  });
  ...
  try {
    const data = await portalFoundationService.listNotifications(auth.userId, parsed.data);
