...
import { listExamNotificationDispatchesSchema } from "@/lib/validators/examCoreSchemas";
...
export async function GET(request: NextRequest) {
  ...
  const parsed = listExamNotificationDispatchesSchema.safeParse({
  ...
  try {
    const result = await examNotificationService.listNotificationDispatches({
    ...
    const message = error instanceof Error ? error.message : "Failed to list exam notification status";
