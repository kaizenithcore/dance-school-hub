...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const certificate = await examCertificateService.getCertificateById(id, auth.user.id);
    return ok(certificate, 200, origin);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to fetch exam certificate";
