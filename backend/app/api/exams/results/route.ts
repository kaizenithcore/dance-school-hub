...
import { upsertExamResultSchema } from "@/lib/validators/examCoreSchemas";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const parsed = upsertExamResultSchema.safeParse(body);

    if (!parsed.success) {
      return fail({ code: "invalid_request", message: "Invalid payload", details: parsed.error.flatten() }, 400, origin);
    }

    const saved = await examEvaluationService.upsertResult(auth.user.id, parsed.data);
    return ok(saved, 200, origin);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to save exam result";
